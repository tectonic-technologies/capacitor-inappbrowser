...
@objc public protocol WKWebViewControllerDelegate {
    @objc optional func webViewController(_ controller: WKWebViewController, canDismiss url: URL)
        -> Bool
    ...
    @objc optional func webViewController(
        _ controller: WKWebViewController, didFail url: URL, withError error: Error)
    @objc optional func webViewController(
...
open class WKWebViewController: UIViewController, WKScriptMessageHandler {
    ...
    public init(
        url: URL, headers: [String: String], isInspectable: Bool,
        credentials: WKWebViewCredentials? = nil, preventDeeplink: Bool
    ) {
        super.init(nibName: nil, bundle: nil)
        self.source = .remote(url)
        self.credentials = credentials
    ...
    public init(
        url: URL, headers: [String: String], isInspectable: Bool,
        credentials: WKWebViewCredentials? = nil, preventDeeplink: Bool, blankNavigationTab: Bool,
        cookies: [[String: Any]]?
    ) {
        super.init(nibName: nil, bundle: nil)
        ...
        // 🍪 Immediately parse and store valid cookies
        self.cookies = parseCookies(cookies)

        self.initWebview(isInspectable: isInspectable)
    }

    private func parseCookies(_ rawCookies: [[String: Any]]?) -> [HTTPCookie] {
        guard let rawCookies = rawCookies else { return [] }

        var result: [HTTPCookie] = []

        for dict in rawCookies {
            guard
            ...
            if let maxAge = dict["maxAge"] as? Int {
                props[.maximumAge] = String(maxAge)
            }
    ...
    fileprivate lazy var backBarButtonItem: UIBarButtonItem = {
        let navBackImage = UIImage(systemName: "chevron.backward")?.withRenderingMode(
            .alwaysTemplate)
        let barButtonItem = UIBarButtonItem(
            image: navBackImage, style: .plain, target: self,
    ...
    fileprivate lazy var forwardBarButtonItem: UIBarButtonItem = {
        let forwardImage = UIImage(systemName: "chevron.forward")?.withRenderingMode(
            .alwaysTemplate)
        let barButtonItem = UIBarButtonItem(
            image: forwardImage, style: .plain, target: self,
    ...
    fileprivate lazy var stopBarButtonItem: UIBarButtonItem = {
        if let image = stopBarButtonItemImage {
            return UIBarButtonItem(
                image: image, style: .plain, target: self, action: #selector(stopDidClick(sender:)))
        } else {
            return UIBarButtonItem(
                barButtonSystemItem: .stop, target: self, action: #selector(stopDidClick(sender:)))
        }
    }()
    ...
    fileprivate lazy var doneBarButtonItem: UIBarButtonItem = {
        if showArrowAsClose {
            ...
            let chevronImage = UIImage(systemName: "chevron.left")?.withRenderingMode(
                .alwaysTemplate)
            let barButtonItem = UIBarButtonItem(
                image: chevronImage, style: .plain, target: self,
            ...
            let barButtonItem = UIBarButtonItem(
                image: xImage, style: .plain, target: self, action: #selector(doneDidClick(sender:))
            )
            if let tintColor = self.tintColor ?? self.navigationController?.navigationBar.tintColor
            {
                barButtonItem.tintColor = tintColor
    ...
    func updateButtonTintColors() {
        ...
        if let tintColor = navigationController?.navigationBar.tintColor {
            ...
            if buttonNearDoneIcon != nil && navigationItem.rightBarButtonItems?.count == 1
    ...
    override open func traitCollectionDidChange(_ previousTraitCollection: UITraitCollection?) {
        ...
        if traitCollection.hasDifferentColorAppearance(comparedTo: previousTraitCollection) {
            ...
            if let navBar = navigationController?.navigationBar {
                if navBar.backgroundColor == UIColor.black
    ...
    open func postMessageToJS(message: [String: Any]) {
        ...
        {
            let script =
                "window.dispatchEvent(new CustomEvent('messageFromNative', { detail: \(jsonString) }));"
        ...
        if message.name == "messageHandler" {
            if let messageBody = message.body as? [String: Any] {
                ...
                self.capBrowserPlugin?.notifyListeners(
                    "messageFromWebview", data: ["rawMessage": String(describing: message.body)])
            }
    ...
    open func initWebview(isInspectable: Bool = true) {
        ...
        self.webView?.customUserAgent =
            self.customUserAgent ?? self.userAgent ?? self.originalUserAgent

        self.navigationItem.title = self.navigationItem.title ?? self.source?.absoluteString

        if let navigation = self.navigationController {
            self.previousNavigationBarState = (
                navigation.navigationBar.tintColor, navigation.navigationBar.isHidden
            )
    ...
    override open func viewWillAppear(_ animated: Bool) {
        ...
        if let tintColor = self.tintColor {
            ...
            if let navController = navigationController,
                let backgroundColor = navController.navigationBar.backgroundColor
    ...
    override open func viewDidAppear(_ animated: Bool) {
        ...
        if buttonNearDoneIcon != nil {
            ...
            let buttonExists =
                navigationItem.rightBarButtonItems?.contains { item in
    ...
    override open func observeValue(
        ...
        case estimatedProgressKeyPath?:
            DispatchQueue.main.async {
                ...
                if estimatedProgress >= 1.0 {
                    UIView.animate(
                        withDuration: 0.3, delay: 0.3, options: .curveEaseOut,
                        animations: {
        ...
        case "URL":
            ...
            self.capBrowserPlugin?.notifyListeners(
                "urlChangeEvent", data: ["url": webView?.url?.absoluteString ?? ""])
...
extension WKWebViewController {
    ...
    public func load(source s: WKWebSource) {
        switch s {
        case .remote(let url):
            self.load(remote: url)
        case .file(let url, let access):
            self.load(file: url, access: access)
        case .string(let str, let base):
            self.load(string: str, base: base)
        }
    }

    public func load(remote: URL) {
        DispatchQueue.main.async {
            self.webView?.load(self.createRequest(url: remote))
        }
    }

    public func load(file: URL, access: URL) {
        webView?.loadFileURL(file, allowingReadAccessTo: access)
    }

    public func load(string: String, base: URL? = nil) {
        webView?.loadHTMLString(string, baseURL: base)
    }

    public func goBackToFirstPage() {
    ...
    public func executeScript(script: String, completion: ((Any?, Error?) -> Void)? = nil) {
        DispatchQueue.main.async { [weak self] in
            self?.webView?.evaluateJavaScript(script, completionHandler: completion)
        }
    }

    public func applyTextZoom(_ zoomPercent: Int) {
        let script = """
...
extension WKWebViewController {
    fileprivate var availableCookies: [HTTPCookie]? {
    ...
    fileprivate func createRequest(url: URL) -> URLRequest {
    ...
    fileprivate func setUpConstraints() {
        if !(self.navigationController?.navigationBar.isHidden)! {
            self.progressView?.frame.origin.y = CGFloat(
    ...
    fileprivate func addBarButtonItems() {
        func barButtonItem(_ type: BarButtonItemType) -> UIBarButtonItem? {
            ...
            case .custom(let icon, let title, let action):
                ...
                if let icon = icon {
                    item = BlockBarButtonItem(
                        image: icon, style: .plain, target: self,
                        action: #selector(customDidClick(sender:)))
                } else {
                    item = BlockBarButtonItem(
                        title: title, style: .plain, target: self,
                        action: #selector(customDidClick(sender:)))
                }
                item.block = action
                return item
                ...
                || rightBarButtons.contains(doneBarButtonItem))
        {
            ...
            if !buttonExists {
                ...
                print(
                    "[DEBUG] Added buttonNearDone to right bar buttons, icon: \(String(describing: buttonNearDoneIcon))"
                )
            } else {
    ...
    fileprivate func rollbackState() {
        progressView?.progress = 0

        navigationController?.navigationBar.tintColor = previousNavigationBarState.tintColor

        navigationController?.setNavigationBarHidden(
            previousNavigationBarState.hidden, animated: true)
    }

    fileprivate func checkRequestCookies(_ request: URLRequest, cookies: [HTTPCookie]) -> Bool {
        if cookies.count <= 0 {
            return true
        }
        guard let headerFields = request.allHTTPHeaderFields,
            let cookieString = headerFields[cookieKey]
        else {
            return false
        }

        let requestCookies = cookieString.components(separatedBy: ";").map {
            $0.trimmingCharacters(in: .whitespacesAndNewlines).split(separator: "=", maxSplits: 1)
                .map(String.init)
        }
    ...
    fileprivate func openURLWithApp(_ url: URL) -> Bool {
    ...
    fileprivate func handleURLWithApp(_ url: URL, targetFrame: WKFrameInfo?) -> Bool {
    ...
    @objc fileprivate func backDidClick(sender: AnyObject) {
    ...
    @objc fileprivate func forwardDidClick(sender: AnyObject) {
        webView?.goForward()
    }

    @objc fileprivate func buttonNearDoneDidClick(sender: AnyObject) {
        self.capBrowserPlugin?.notifyListeners("buttonNearDoneClick", data: [:])
    }

    @objc fileprivate func reloadDidClick(sender: AnyObject) {
        webView?.stopLoading()
        if webView?.url != nil {
            webView?.reload()
    ...
    @objc fileprivate func stopDidClick(sender: AnyObject) {
        webView?.stopLoading()
    }

    @objc fileprivate func activityDidClick(sender: AnyObject) {
    ...
    fileprivate func showDisclaimer(items: [Any], sender: AnyObject) {
    ...
    fileprivate func closeView() {
        ...
        if canDismiss {
            ...
            webView?.configuration.userContentController.removeScriptMessageHandler(
                forName: "messageHandler")
            webView?.configuration.userContentController.removeScriptMessageHandler(
                forName: "close")
            webView?.configuration.userContentController.removeScriptMessageHandler(
                forName: "preShowScriptSuccess")
            webView?.configuration.userContentController.removeScriptMessageHandler(
                forName: "preShowScriptError")
            webView = nil

            self.capBrowserPlugin?.notifyListeners(
                "closeEvent", data: ["url": webView?.url?.absoluteString ?? ""])
            dismiss(animated: true, completion: nil)
        }
    }

    @objc fileprivate func doneDidClick(sender: AnyObject) {
        // check if closeModal is true, if true display alert before close
        if self.closeModal {
            let alert = UIAlertController(
                title: self.closeModalTitle, message: self.closeModalDescription,
                preferredStyle: UIAlertController.Style.alert)
            alert.addAction(
                UIAlertAction(
                    title: self.closeModalOk, style: UIAlertAction.Style.default,
                    handler: { _ in
                        self.closeView()
                    }))
            alert.addAction(
                UIAlertAction(
                    title: self.closeModalCancel, style: UIAlertAction.Style.default, handler: nil))
            self.present(alert, animated: true, completion: nil)
        } else {
            self.closeView()
        }
    ...
    @objc fileprivate func customDidClick(sender: BlockBarButtonItem) {
        sender.block?(self)
    }

    fileprivate func canRotate() {}
    ...
    fileprivate open func setUpNavigationBarAppearance() {
...
extension WKWebViewController: WKNavigationDelegate {
    internal func injectPreShowScript() {
        ...
        if self.preShowSemaphore?.wait(timeout: .now() + 10) == .timedOut {
            print(
                "[InAppBrowser - InjectPreShowScript] PreShowScript running for over 10 seconds. The plugin will not wait any longer!"
            )
            return
        }
    ...
    public func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        ...
        } else if self.preShowScript != nil && !self.preShowScript!.isEmpty
    ...
    public func webView(
        ...
        if let credentials = credentials,
            ...
            let url = webView.url, challenge.protectionSpace.host == url.host,
            challenge.protectionSpace.protocol == url.scheme,
            challenge.protectionSpace.port == url.port
                ?? (url.scheme == "https" ? 443 : url.scheme == "http" ? 80 : nil)
        {
            let urlCredential = URLCredential(
                user: credentials.username, password: credentials.password, persistence: .none)
            completionHandler(.useCredential, urlCredential)
        } else if let bypassedSSLHosts = bypassedSSLHosts,
            bypassedSSLHosts.contains(challenge.protectionSpace.host)
        {
    ...
    public func webView(
        ...
        if u.host == self.source?.url?.host, let cookies = availableCookies,
        ...
        if let navigationType = NavigationType(rawValue: navigationAction.navigationType.rawValue),
            let result = delegate?.webViewController?(
                self, decidePolicy: u, navigationType: navigationType)
        {
